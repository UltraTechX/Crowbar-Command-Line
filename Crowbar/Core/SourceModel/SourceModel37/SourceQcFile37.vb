...
Public Class SourceQcFile37
	Inherits SourceQcFile

#Region "Creation and Destruction"

	Public Sub New(ByVal outputFileStream As StreamWriter, ByVal outputPathFileName As String, ByVal mdlFileData As SourceMdlFileData37, ByVal phyFileData As SourcePhyFileData, ByVal vtxFileData As SourceVtxFileData06, ByVal modelName As String)
...
#Region "Private Methods"
		...
		If Me.theMdlFileData.theSkinFamilies IsNot Nothing AndAlso Me.theMdlFileData.theSkinFamilies.Count > 0 AndAlso Me.theMdlFileData.theTextures IsNot Nothing AndAlso Me.theMdlFileData.theTextures.Count > 0 AndAlso Me.theMdlFileData.skinReferenceCount > 0 Then
			...
			For i As Integer = 0 To processedSkinFamilies.Count - 1
				...
				For j As Integer = 0 To skinReferenceCount - 1
					Dim aTexture As SourceMdlTexture37
	...
	Public Sub WriteIncludeModelCommands()
		...
		If Me.theMdlFileData.theIncludeModelFileNames IsNot Nothing Then
			line = ""
			Me.theOutputFileStreamWriter.WriteLine(line)

			For i As Integer = 0 To Me.theMdlFileData.theIncludeModelFileNames.Count - 1
				Dim anIncludeModelFileName As String
				anIncludeModelFileName = Me.theMdlFileData.theIncludeModelFileNames(i)
				If Not String.IsNullOrEmpty(anIncludeModelFileName) Then
					If TheApp.Settings.DecompileQcUseMixedCaseForKeywordsIsChecked Then
						line = "$IncludeModel "
					Else
						line = "$includemodel "
					End If
					line += """"
					If anIncludeModelFileName.StartsWith("models/") Then
	...
	Private Sub FillInWeightLists()
		If Me.theMdlFileData.theSequenceDescs IsNot Nothing Then
			Dim aSeqDesc As SourceMdlSequenceDesc37
	...
	Private Sub WriteAnimationLine(ByVal anAnimationDesc As SourceMdlAnimationDesc37)
		Dim line As String = ""
		...
		Else
			...
			End If
			line += " """
			If Not anAnimationDesc.theName.StartsWith("a_") Then
				line += "a_"
			End If
	...
	Private Sub WriteSequenceOrDeclareSequenceCommand()
		...
		If Me.theMdlFileData.theSequenceDescs IsNot Nothing Then
			For i As Integer = 0 To Me.theMdlFileData.theSequenceDescs.Count - 1
				Dim aSequenceDesc As SourceMdlSequenceDesc37
	...
	Private Sub WriteSequenceLine(ByVal aSequenceDesc As SourceMdlSequenceDesc37)
		Dim line As String = ""
	...
	Private Sub WriteSequenceOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc37)
		...
		If aSequenceDesc.theIkLocks IsNot Nothing AndAlso Me.theMdlFileData.theIkChains IsNot Nothing Then
			Dim ikLock As SourceMdlIkLock37
	...
	'lookupControl( token )       
	Private Sub WriteAnimationOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc37, ByVal anAnimationDesc As SourceMdlAnimationDesc37, ByVal impliedAnimDesc As SourceMdlAnimationDesc37)
		Dim line As String = ""
	...
	'worldspaceblend       //
	'worldspaceblendloop   // 
	Private Sub WriteCmdListOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc37, ByVal anAnimationDesc As SourceMdlAnimationDesc37, ByVal impliedAnimDesc As SourceMdlAnimationDesc37)
		Dim line As String = ""
		...
		' weightlist "top_bottom"
		Dim aSeqDesc As SourceMdlSequenceDesc37 = Nothing
		If aSequenceDesc Is Nothing Then
	...
	Private Sub WriteSequenceWeightListLine(ByVal aSeqDesc As SourceMdlSequenceDesc37)
		Dim line As String = ""
	...
	Private Sub WriteSequenceBlendInfo(ByVal aSeqDesc As SourceMdlSequenceDesc37)
		Dim line As String = ""
	...
	Private Sub WriteSequenceDeltaInfo(ByVal aSeqDesc As SourceMdlSequenceDesc37)
		Dim line As String = ""
	...
	Private Sub WriteSequenceLayerInfo(ByVal aSeqDesc As SourceMdlSequenceDesc37)
	...
	Private Sub WriteSequenceNodeInfo(ByVal aSeqDesc As SourceMdlSequenceDesc37)
		Dim line As String = ""
	...
	Private Sub WriteHBoxRelatedCommands()
		...
		Dim aHitboxSet As SourceMdlHitboxSet37
	...
	Private Sub WriteHBoxCommands(ByVal theHitboxes As List(Of SourceMdlHitbox37), ByVal commentTag As String, ByVal hitboxSetName As String, ByRef theSkipBoneInBBoxCommandWasUsed As Boolean)
		Dim line As String = ""
		Dim aHitbox As SourceMdlHitbox37
	...
	Public Sub WriteControllerCommand()
		Dim line As String = ""
		Dim boneController As SourceMdlBoneController37
