	...
	Public Sub ReadBodyParts()
		If Me.theMdlFileData.bodyPartCount > 0 Then
			...
			Try
				...
				Me.theMdlFileData.theBodyParts = New List(Of SourceMdlBodyPart10)(Me.theMdlFileData.bodyPartCount)
				For bodyPartIndex As Integer = 0 To Me.theMdlFileData.bodyPartCount - 1
					...
					Dim aBodyPart As New SourceMdlBodyPart10()
	...
	Public Sub ReadTextures()
		If Me.theMdlFileData.textureCount > 0 Then
			...
			Try
				...
				Me.theMdlFileData.theTextures = New List(Of SourceMdlTexture10)(Me.theMdlFileData.textureCount)
...
#Region "Private Methods"
	...
	Private Sub ReadModels(ByVal aBodyPart As SourceMdlBodyPart10)
		...
		Try
			...
			aBodyPart.theModels = New List(Of SourceMdlModel10)(aBodyPart.modelCount)
			For bodyPartIndex As Integer = 0 To aBodyPart.modelCount - 1
				...
				Dim aModel As New SourceMdlModel10()
				...
				aModel.groupCount = Me.theInputFileReader.ReadInt32()
				aModel.groupOffset = Me.theInputFileReader.ReadInt32()
	...
	Private Sub ReadModelVertexBoneInfos(ByVal aModel As SourceMdlModel10)
	...
	Private Sub ReadModelNormalBoneInfos(ByVal aModel As SourceMdlModel10)
	...
	Private Sub ReadModelVertexes(ByVal aModel As SourceMdlModel10)
		If aModel.vertexCount > 0 Then
			Dim fileOffsetStart As Long
			Dim fileOffsetEnd As Long
	...
	Private Sub ReadModelNormals(ByVal aModel As SourceMdlModel10)
		If aModel.normalCount > 0 Then
			Dim fileOffsetStart As Long
			Dim fileOffsetEnd As Long
	...
	Private Sub ReadMeshes(ByVal aModel As SourceMdlModel10)
		If aModel.meshCount > 0 Then
			...
			Try
				...
				aModel.theMeshes = New List(Of SourceMdlMesh10)(aModel.meshCount)
				For meshIndex As Integer = 0 To aModel.meshCount - 1
					fileOffsetStart = Me.theInputFileReader.BaseStream.Position
					Dim aMesh As New SourceMdlMesh10()
	...
	Private Sub ReadFaces(ByVal aMesh As SourceMdlMesh10)
	...
	Private Sub ReadTextureData(ByVal aTexture As SourceMdlTexture10)
		'Dim boneInputFileStreamPosition As Long
		'Dim inputFileStreamPosition As Long
		Dim fileOffsetStart As Long
		Dim fileOffsetEnd As Long
...
#Region "Data"
	...
	Protected theMdlFileData As SourceMdlFileData10
