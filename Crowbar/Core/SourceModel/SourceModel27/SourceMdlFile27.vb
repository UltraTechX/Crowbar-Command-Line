...
Public Class SourceMdlFile27

#Region "Creation and Destruction"

	Public Sub New(ByVal mdlFileReader As BinaryReader, ByVal mdlFileData As SourceMdlFileData27)
	...
	'Public Sub New(ByVal mdlFileWriter As BinaryWriter, ByVal mdlFileData As SourceMdlFileData27)
...
#Region "Methods"
	...
	Public Sub ReadMdlHeader01(ByVal logDescription As String)
		...
		'DEBUG: This is filler to get the known boneOffset read in.
	...
	Public Sub ReadHitboxes()
		If Me.theMdlFileData.hitboxCount > 0 Then
			...
			Try
				...
				Me.theMdlFileData.theHitboxes = New List(Of SourceMdlHitbox31)(Me.theMdlFileData.hitboxCount)
				For j As Integer = 0 To Me.theMdlFileData.hitboxCount - 1
				...
				fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
				Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, "theMdlFileData.theHitboxes " + Me.theMdlFileData.theHitboxes.Count.ToString())

				Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 4, "theMdlFileData.theHitboxes alignment")
			Catch ex As Exception
				Dim debug As Integer = 4242
			End Try
		End If
	...
	Public Sub ReadSequences()
		If Me.theMdlFileData.localSequenceCount > 0 Then
			...
			Dim fileOffsetStart2 As Long
			Dim fileOffsetEnd2 As Long
			Dim unknownbytes As Int32

			Try
				Me.theInputFileReader.BaseStream.Seek(Me.theMdlFileData.localSequenceOffset, SeekOrigin.Begin)
				'fileOffsetStart = Me.theInputFileReader.BaseStream.Position

				Me.theMdlFileData.theSequenceDescs = New List(Of SourceMdlSequenceDesc31)(Me.theMdlFileData.localSequenceCount)
				For i As Integer = 0 To Me.theMdlFileData.localSequenceCount - 1
					...
					fileOffsetStart = Me.theInputFileReader.BaseStream.Position
					Dim aSeqDesc As New SourceMdlSequenceDesc31()
					...
					'Next
					aSeqDesc.unused(0) = Me.theInputFileReader.ReadInt32()

					'NOTE: Not sure why these 4060 bytes were ever included; they are always zeroes.
					inputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
					Me.theInputFileReader.BaseStream.Seek(inputFileStreamPosition + 4060, SeekOrigin.Begin)
	...
	Public Sub ReadSequenceGroups()
		If Me.theMdlFileData.sequenceGroupCount > 0 Then
			...
			Try
				...
				Me.theMdlFileData.theSequenceGroups = New List(Of SourceMdlSequenceGroup31)(Me.theMdlFileData.sequenceGroupCount)
				For sequenceGroupIndex As Integer = 0 To Me.theMdlFileData.sequenceGroupCount - 1
					...
					Dim aSequenceGroup As New SourceMdlSequenceGroup31()
	...
	Public Sub ReadLocalAnimationDescs()
		If Me.theMdlFileData.animationCount > 0 Then
			...
			Try
				...
				Me.theMdlFileData.theAnimationDescs = New List(Of SourceMdlAnimationDesc31)(Me.theMdlFileData.animationCount)
				For i As Integer = 0 To Me.theMdlFileData.animationCount - 1
					...
					'fileOffsetStart = Me.theInputFileReader.BaseStream.Position
					Dim anAnimationDesc As New SourceMdlAnimationDesc31()
					...
					If anAnimationDesc.nameOffset <> 0 Then
						...
						Dim animNameInFile As String
						animNameInFile = FileManager.ReadNullTerminatedString(Me.theInputFileReader)
						If animNameInFile(0) = "@" Then
							anAnimationDesc.theName = animNameInFile.Remove(0, 1)
						Else
							anAnimationDesc.theName = animNameInFile
						End If

						fileOffsetEnd2 = Me.theInputFileReader.BaseStream.Position - 1
						If Not Me.theMdlFileData.theFileSeekLog.ContainsKey(fileOffsetStart2) Then
							Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart2, fileOffsetEnd2, "anAnimationDesc.theName [in file] = " + animNameInFile)
						End If
					Else
	...
	Public Sub ReadTextures()
		If Me.theMdlFileData.textureCount > 0 Then
			...
			Me.theMdlFileData.theTextures = New List(Of SourceMdlTexture31)(Me.theMdlFileData.textureCount)
			For i As Integer = 0 To Me.theMdlFileData.textureCount - 1
				...
				Dim aTexture As New SourceMdlTexture31()
...
#Region "Private Methods"
	...
	Private Sub ReadAnimations(ByVal animationDescInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc31)
		If anAnimationDesc.animOffset > 0 Then
			...
			Try
				...
				anAnimationDesc.theAnimations = New List(Of SourceMdlAnimation31)(Me.theMdlFileData.theBones.Count)
				For boneIndex As Integer = 0 To Me.theMdlFileData.theBones.Count - 1
					...
					fileOffsetStart = Me.theInputFileReader.BaseStream.Position
					Dim anAnimation As New SourceMdlAnimation31()
		...
		If aBodyPart.modelCount > 0 Then
			...
			Try
				...
				For j As Integer = 0 To aBodyPart.modelCount - 1
					...
					aModel.normalOffset = Me.theInputFileReader.ReadInt32()
					aModel.tangentOffset = Me.theInputFileReader.ReadInt32()
					aModel.texCoordOffset = Me.theInputFileReader.ReadInt32()
					aModel.boneWeightsOffset = Me.theInputFileReader.ReadInt32()
					...
					Me.ReadBoneWeights(modelInputFileStreamPosition, aModel)
					Me.ReadNormals(modelInputFileStreamPosition, aModel)

					'fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
					''NOTE: Although studiomdl source code indicates ALIGN64, it seems to align on 32.
					'Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 64, "aModel.theVertexes pre-alignment (NOTE: Should end at: " + CStr(modelInputFileStreamPosition + aModel.vertexOffset - 1) + ")")
					...
					Me.ReadTexCoords(modelInputFileStreamPosition, aModel)
		...
		If aModel.vertexCount > 0 AndAlso aModel.boneWeightsOffset <> 0 Then
			...
			Try
				...
				For vertexIndex As Integer = 0 To aModel.vertexCount - 1
					...
					aBoneWeight.bone(2) = Me.theInputFileReader.ReadInt32()
					aBoneWeight.bone(3) = Me.theInputFileReader.ReadInt32()
					aBoneWeight.weight(0) = Me.theInputFileReader.ReadSingle()
					aBoneWeight.weight(1) = Me.theInputFileReader.ReadSingle()
					aBoneWeight.weight(2) = Me.theInputFileReader.ReadSingle()
					aBoneWeight.weight(3) = Me.theInputFileReader.ReadSingle()
					aBoneWeight.boneCount = Me.theInputFileReader.ReadInt16()
					aBoneWeight.material = Me.theInputFileReader.ReadInt16()
					aBoneWeight.firstRef = Me.theInputFileReader.ReadInt16()
					aBoneWeight.lastRef = Me.theInputFileReader.ReadInt16()

					aModel.theBoneWeights.Add(aBoneWeight)
				...
				fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
				Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, "aModel.theBoneWeights " + aModel.theBoneWeights.Count.ToString())
	...
	Private Sub ReadVertexes(ByVal modelInputFileStreamPosition As Long, ByVal aModel As SourceMdlModel27)
		If aModel.vertexCount > 0 AndAlso aModel.vertexOffset <> 0 Then
			...
			Try
				...
				aModel.theVertexes = New List(Of SourceMdlVertex31)(aModel.vertexCount)
				For vertexIndex As Integer = 0 To aModel.vertexCount - 1
					vertexInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
					Dim aVertex As New SourceMdlVertex31()
	...
	Private Sub ReadNormals(ByVal modelInputFileStreamPosition As Long, ByVal aModel As SourceMdlModel27)
		If aModel.vertexCount > 0 AndAlso aModel.normalOffset <> 0 Then
			...
			Try
				Me.theInputFileReader.BaseStream.Seek(modelInputFileStreamPosition + aModel.normalOffset, SeekOrigin.Begin)
				fileOffsetStart = Me.theInputFileReader.BaseStream.Position

				aModel.theNormals = New List(Of SourceMdlVertex31)(aModel.vertexCount)
				For vertexIndex As Integer = 0 To aModel.vertexCount - 1
					vertexInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
					Dim aNormal As New SourceMdlVertex31()

					aNormal.normal.x = Me.theInputFileReader.ReadSingle()
	...
	Private Sub ReadTexCoords(ByVal modelInputFileStreamPosition As Long, ByVal aModel As SourceMdlModel27)
		If aModel.vertexCount > 0 AndAlso aModel.texCoordOffset <> 0 Then
			...
			Try
				Me.theInputFileReader.BaseStream.Seek(modelInputFileStreamPosition + aModel.texCoordOffset, SeekOrigin.Begin)
				fileOffsetStart = Me.theInputFileReader.BaseStream.Position

				aModel.theTexCoords = New List(Of SourceMdlVertex31)(aModel.vertexCount)
				For vertexIndex As Integer = 0 To aModel.vertexCount - 1
					vertexInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
					Dim aVertex As New SourceMdlVertex31()
					...
					aModel.theTexCoords.Add(aVertex)
				...
				fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
				Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, "aModel.theTexCoords " + aModel.theTexCoords.Count.ToString())
...
#Region "Data"
	...
	Protected theMdlFileData As SourceMdlFileData27
