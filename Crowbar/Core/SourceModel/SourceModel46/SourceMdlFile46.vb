...
Public Class SourceMdlFile46

#Region "Creation and Destruction"

	Public Sub New(ByVal mdlFileReader As BinaryReader, ByVal mdlFileData As SourceMdlFileData46)
	...
	Public Sub New(ByVal mdlFileWriter As BinaryWriter, ByVal mdlFileData As SourceMdlFileData46)
...
#Region "Methods"
	...
	Public Sub ReadMdlHeader02(ByVal logDescription As String)
		...
		If Me.theMdlFileData.nameOffset > 0 Then
			inputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
			...
			fileOffsetEnd2 = Me.theInputFileReader.BaseStream.Position - 1
			If Not Me.theMdlFileData.theFileSeekLog.ContainsKey(fileOffsetStart2) Then
				Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart2, fileOffsetEnd2, "theName")
			End If
			Me.theInputFileReader.BaseStream.Seek(inputFileStreamPosition, SeekOrigin.Begin)
		End If
	...
	Public Sub ReadLocalAnimationDescs()
		...
		Dim animationDescSize As Long

		Me.theInputFileReader.BaseStream.Seek(Me.theMdlFileData.localAnimationOffset, SeekOrigin.Begin)
		fileOffsetStart = Me.theInputFileReader.BaseStream.Position

		Me.theMdlFileData.theAnimationDescs = New List(Of SourceMdlAnimationDesc46)(Me.theMdlFileData.localAnimationCount)
		For i As Integer = 0 To Me.theMdlFileData.localAnimationCount - 1
			...
			Dim anAnimationDesc As New SourceMdlAnimationDesc46()
		...
		' Fill-in the byte alignments. Can not be done in the previous loop because need to read all AnimationDescs first.
		For i As Integer = 0 To Me.theMdlFileData.localAnimationCount - 1
			...
			If ((anAnimationDesc.flags And SourceMdlAnimationDesc.STUDIO_ALLZEROS) = 0) Then
				...
				Else
					If anAnimationDesc.sectionOffset <> 0 AndAlso anAnimationDesc.sectionFrameCount > 0 Then
						...
						If anAnimationDesc.animBlock = 0 Then
							For sectionIndex As Integer = 0 To sectionCount - 1
								...
								'fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
								'Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 16, "anAnimationDesc.theAnimations pre-alignment")
					...
					ElseIf anAnimationDesc.animBlock = 0 Then
						If anAnimationDesc.animOffset > 0 Then
							fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
							Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 8, "anAnimationDesc.theAnimations pre-alignment (NOTE: Should end at: " + CStr(animInputFileStreamPosition + anAnimationDesc.animOffset - 1) + ")")
						End If
				...
				If anAnimationDesc.animBlock = 0 AndAlso anAnimationDesc.ikRuleCount > 0 Then
					'fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
					''NOTE: This seems to align on 4 even though source code indicates otherwise. Also, still a gap for TF2 engineer, sniper, and spy.
					'Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 4, "anAnimationDesc.theIkRules pre-alignment")

					Me.ReadMdlIkRules(animInputFileStreamPosition, anAnimationDesc)
				End If

				If anAnimationDesc.animBlock = 0 AndAlso anAnimationDesc.localHierarchyCount > 0 Then
	...
	Protected Sub ReadMdlAnimation(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc46, ByVal sectionFrameCount As Integer, ByVal aSectionOfAnimation As List(Of SourceMdlAnimation))
		...
		For j As Integer = 0 To boneCount - 1
			...
			If boneIndex = 255 Then
				...
				Me.theMdlFileData.theFileSeekLog.Add(animationInputFileStreamPosition, Me.theInputFileReader.BaseStream.Position - 1, "anAnimationDesc.anAnimation (boneIndex = 255)")
		...
		If boneIndex <> 255 Then
			...
			fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
			Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, "anAnimationDesc.anAnimation (unused empty data structure at the end of the list)")
		End If

		fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
		Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 4, "anAnimationDesc.anAnimation alignment")
	End Sub
	...
	Protected Sub ReadMdlIkRules(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc46)
	...
	Protected Sub ReadMdlAnimationSection(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc46, ByVal aFileSeekLog As FileSeekLog)
	...
	Protected Sub ReadMdlMovements(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc46)
	...
	Protected Sub ReadLocalHierarchies(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc46)
	...
	Private Sub ReadActivityModifiers(ByVal seqInputFileStreamPosition As Long, ByVal aSeqDesc As SourceMdlSequenceDesc)
		...
		'Me.theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(Me.theInputFileReader, fileOffsetEnd, 4, "aSeqDesc.theActivityModifiers alignment")
	End Sub
		...
		If Me.theMdlFileData.sourceBoneTransformCount > 0 Then
			...
			For i As Integer = 0 To Me.theMdlFileData.sourceBoneTransformCount - 1
				...
				If aBoneTransform.nameOffset <> 0 Then
					...
					fileOffsetEnd2 = Me.theInputFileReader.BaseStream.Position - 1
					If Not Me.theMdlFileData.theFileSeekLog.ContainsKey(fileOffsetStart2) Then
						Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart2, fileOffsetEnd2, "aBoneTransform.theName")
					End If
			...
			fileOffsetEnd = Me.theInputFileReader.BaseStream.Position - 1
			Me.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, "theMdlFileData.theBoneTransforms")
...
#Region "Data"
	...
	Protected theMdlFileData As SourceMdlFileData46
