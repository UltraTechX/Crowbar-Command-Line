...
#Region "Methods"
	...
	Public Sub ReadMdlHeader01(ByVal logDescription As String)
		...
		For x As Integer = 0 To Me.theMdlFileData.unused.Length - 1
			Me.theMdlFileData.unused(x) = Me.theInputFileReader.ReadByte()
		Next

		Me.theMdlFileData.zeroframecacheindex = Me.theInputFileReader.ReadInt32()
		...
		For x As Integer = 0 To Me.theMdlFileData.unused2.Length - 1
			Me.theMdlFileData.unused2(x) = Me.theInputFileReader.ReadInt32()
		Next
	...
	Private Sub ReadHitboxes(ByVal hitboxOffsetInputFileStreamPosition As Long, ByVal aHitboxSet As SourceMdlHitboxSet)
		If aHitboxSet.hitboxCount > 0 Then
			...
			For j As Integer = 0 To aHitboxSet.hitboxCount - 1
				...
				'NOTE: I looked at many models from various games that use MDL v44 and none of them had anything for the name, 
				'      so it seems best to ignore the offset and always use an empty name.
				...
				'End If
				'======
				'NOTE: The 1279345491 is ascii for "SCAL" which seems to be in every MDL file in Vindictus.
				If aHitbox.nameOffset <> 0 AndAlso Me.theMdlFileData.unused2(1) = 1279345491 Then
					'NOTE: Contrary to the studiomdl source code for MDL v44, the nameOffset is absolute offset for the following tested models 
					'      from Half-Life 2\hl1\hl1_pak_dir\models: agrunt, scientist
					'Me.theInputFileReader.BaseStream.Seek(aHitbox.nameOffset, SeekOrigin.Begin)
					'NOTE: For Vindictus pet_succubus_tiny.mdl, relative offset is correct.
	...
	Public Sub ReadLocalAnimationDescs()
		...
		Me.theMdlFileData.theAnimationDescs = New List(Of SourceMdlAnimationDesc44)(Me.theMdlFileData.localAnimationCount)
		For i As Integer = 0 To Me.theMdlFileData.localAnimationCount - 1
			...
			Dim anAnimationDesc As New SourceMdlAnimationDesc44()
	...
	Public Sub ReadAnimationSections()
		If Me.theMdlFileData.theAnimationDescs IsNot Nothing Then
			For Each anAnimationDesc As SourceMdlAnimationDesc44 In Me.theMdlFileData.theAnimationDescs
	...
	Protected Sub ReadAnimationDescName(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44)
	...
	Protected Function ReadAnimationDescSpanData(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44) As Long
		Dim fileOffsetStart As Long
	...
	Protected Sub ReadMdlAnimation(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44, ByVal sectionFrameCount As Integer, ByVal aSectionOfAnimation As List(Of SourceMdlAnimation), ByVal lastSectionIsBeingRead As Boolean)
		...
		For j As Integer = 0 To boneCount - 1
			...
			Me.theMdlFileData.theFileSeekLog.Add(animationInputFileStreamPosition, Me.theInputFileReader.BaseStream.Position - 1, "anAnimationDesc.anAnimation [" + anAnimationDesc.theName + "]")
	...
	Protected Sub ReadMdlIkRules(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44)
	...
	Protected Sub ReadMdlAnimationSection(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44, ByVal aFileSeekLog As FileSeekLog)
	...
	Protected Sub ReadMdlMovements(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44)
	...
	Protected Sub ReadLocalHierarchies(ByVal animInputFileStreamPosition As Long, ByVal anAnimationDesc As SourceMdlAnimationDesc44)
