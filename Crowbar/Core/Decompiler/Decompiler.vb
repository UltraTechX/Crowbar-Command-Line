...
#Region "Private Methods in Background Thread"
	...
	Private Function Decompile() As AppEnums.StatusMessage
        ...
        Console.WriteLine("... " + progressDescriptionText + " finished.")
    ...
    Private Function DecompileOneModel(ByVal mdlPathFileName As String) As AppEnums.StatusMessage
        ...
        Try
            ...
            Try
                ...
                If model IsNot Nothing Then
                    If TheApp.Settings.DecompileOverrideMdlVersion = SupportedMdlVersion.DoNotOverride Then
                        Console.WriteLine("Model version " + CStr(model.Version) + " detected.")
                    Else
                        Console.WriteLine("Model version overridden to be " + CStr(model.Version) + ".")
                    End If
                Else
            ...
            If status = StatusMessage.ErrorInvalidInternalMdlFileSize Then
                Console.WriteLine("WARNING: The internally recorded file size is different than the actual file size. Some data might not decompile correctly.")
            End If
            Console.WriteLine("... Reading MDL file header finished.")
            ...
            If status = StatusMessage.ErrorRequiredMdlFileNotFound _
                ...
                Console.WriteLine("... Decompiling """ + mdlRelativePathFileName + """ stopped due to missing file.")
                Return status
            ElseIf status = StatusMessage.ErrorInvalidMdlFileId Then
                Console.WriteLine("... Decompiling """ + mdlRelativePathFileName + """ stopped due to invalid file.")
                Return status
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Decompiling """ + mdlRelativePathFileName + """ stopped due to error.")
                Return status
            ElseIf Me.CancellationPending Then
                Console.WriteLine("... Decompiling """ + mdlRelativePathFileName + """ canceled.")
                status = StatusMessage.Canceled
                Return status
            ElseIf Me.theSkipCurrentModelIsActive Then
                Console.WriteLine("... Skipping """ + mdlRelativePathFileName + """.")
                Return status
            Else
            ...
            If TheApp.Settings.DecompileDebugInfoFilesIsChecked Then
                Console.WriteLine("Writing decompile-info files ...")
                Me.WriteDebugFiles(model)
                If Me.CancellationPending Then
                    Console.WriteLine("... Decompile of """ + mdlRelativePathFileName + """ canceled.")
                    status = StatusMessage.Canceled
                    Return status
                ElseIf Me.theSkipCurrentModelIsActive Then
                    Console.WriteLine("... Skipping """ + mdlRelativePathFileName + """.")
                    ...
                    Return status
                Else
                    Console.WriteLine("... Writing decompile-info files finished.")
                End If
            End If
            ...
            ElseIf Me.theSkipCurrentModelIsActive Then
                ...
                Return status
            Else
                Console.WriteLine("... Writing data finished.")
            End If

            Console.WriteLine("... Decompiling """ + mdlRelativePathFileName + """ finished.")
    ...
    Private Function ReadCompiledFiles(ByVal mdlPathFileName As String, ByVal model As SourceModel) As AppEnums.StatusMessage
        ...
        Console.WriteLine("Reading MDL file ...")
        status = model.ReadMdlFile()
        If status = StatusMessage.Success Then
            Console.WriteLine("... Reading MDL file finished.")
        ElseIf status = StatusMessage.Error Then
            Console.WriteLine("... Reading MDL file FAILED. (Probably unexpected format.)")
            Return status
        End If

        If model.SequenceGroupMdlFilesAreUsed Then
            Console.WriteLine("Reading sequence group MDL files ...")
            status = model.ReadSequenceGroupMdlFiles()
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading sequence group MDL files finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading sequence group MDL files FAILED. (Probably unexpected format.)")
            End If
        End If

        If model.TextureMdlFileIsUsed Then
            Console.WriteLine("Reading texture MDL file ...")
            status = model.ReadTextureMdlFile()
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading texture MDL file finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading texture MDL file FAILED. (Probably unexpected format.)")
            End If
        End If

        If model.PhyFileIsUsed Then
            Console.WriteLine("Reading PHY file ...")
            ...
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading PHY file finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading PHY file FAILED. (Probably unexpected format.)")
            End If
        End If

        If model.VtxFileIsUsed Then
            Console.WriteLine("Reading VTX file ...")
            status = model.ReadVtxFile()
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading VTX file finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading VTX file FAILED. (Probably unexpected format.)")
            End If
        End If

        If model.AniFileIsUsed AndAlso TheApp.Settings.DecompileBoneAnimationSmdFilesIsChecked Then
            Console.WriteLine("Reading ANI file ...")
            status = model.ReadAniFile()
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading ANI file finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading ANI file FAILED. (Probably unexpected format.)")
            End If
        End If

        If model.VvdFileIsUsed Then
            Console.WriteLine("Reading VVD file ...")
            status = model.ReadVvdFile()
            If status = StatusMessage.Success Then
                Console.WriteLine("... Reading VVD file finished.")
            ElseIf status = StatusMessage.Error Then
                Console.WriteLine("... Reading VVD file FAILED. (Probably unexpected format.)")
            End If
        End If
    ...
    Private Function WriteDeclareSequenceQciFile(ByVal model As SourceModel) As AppEnums.StatusMessage
        ...
        If TheApp.Settings.DecompileDeclareSequenceQciFileIsChecked Then
            If model.HasBoneAnimationData Then
                Console.WriteLine("DeclareSequence QCI file: ")
    ...
    Private Sub UpdateProgress(ByVal indentLevel As Integer, ByVal line As String)
        ...
        Next
        indentedLine += line
        Console.WriteLine(indentedLine)
    End Sub
...
#Region "Event Handlers"
    ...
    Private Sub Model_SourceModelProgress(ByVal sender As Object, ByVal e As SourceModelProgressEventArgs)
        If e.Progress = ProgressOptions.WarningPhyFileChecksumDoesNotMatchMdlFileChecksum Then
            ...
            Console.WriteLine("WARNING: The PHY file's checksum value does not match the MDL file's checksum value.")
